...
if [ -n "$(ls -A $BUIlD_DIR/docs 2>/dev/null)" ]; then
  cp -a $BUILD_DIR/docs/* $PREFIX/$DOC_DIR
  cp $BUILD_DIR/*.txt $PREFIX/$DOC_DIR/
else
  echo "Doc generation is currently disabled in our RPM build. If this is an issue, it should be possible to enable them with some work. See https://git.hubteam.com/HubSpot/apache-hbase/blob/hubspot-2.x-staging/rpm/sources/do-component-build#L17-L24 for details." > $PREFIX/$DOC_DIR/README.txt
fi
